...
namespace ETNA.SGI.Data.Compras.Impl.MsSQL
{
    public class CotizacionMsSQLDAO : CotizacionDAO
    {
        ...
        public DataTable DGetCotizacionAprobacion()
        {
            ...
            string sql = "select codCotizacion ,descripcion , codRequerimiento , fechaExpiracion , b.razonSocial razonSocial, (select sum((x.cantidad * x.precioUnidad ) - x.descuento) from CotizacionDetalle x where x.codCotizacion = a.codCotizacion) as monto from Cotizacion a , proveedor b where b.codProveedor = a.codProveedor and a.codEstado =@codEstado and a.codCotizacion not in (select c.codCotizacion from OrdenCompra c where c.codCotizacion = a.codCotizacion and c.codRequerimiento = a.codRequerimiento ) and a.codRequerimiento not in ( select d.codRequerimiento  from Cotizacion d   where d.codEstado = 2   and d.codRequerimiento = a.codRequerimiento  ) and fechaExpiracion > getdate()";
